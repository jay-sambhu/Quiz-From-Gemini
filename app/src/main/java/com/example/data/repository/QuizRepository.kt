        ...
        val quizSet = QuizSetEntity(
            ...
            description = if (description.isBlank()) {
                if (searchGroundingQuery.isNotBlank()) {
                    "Generated by Gemini AI with Google Search Grounding for '$searchGroundingQuery'"
                } else {
                    "Generated by Gemini AI for topic '$topic'"
                }
            } else description,
            categoryId = categoryId,
            categoryName = categoryName,
    ...
    suspend fun fetchStudentAttemptsFromFirestore(studentId: String): List<QuizAttemptEntity> {
        ...
        return if (!firestoreAttempts.isNullOrEmpty()) {
            ...
            if (studentId.isNotBlank()) {
                ...
                quizDao.getAllAttempts().firstOrNull() ?: emptyList()
            }
        }
    }
